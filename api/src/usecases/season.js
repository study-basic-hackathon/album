...
export async function getSeasonById(seasonId) {
  const result = await seasonRepository.findSeasonById(seasonId);
  return result[0];
};

// 季節の作品一覧の取得
export async function getSeasonWorks(seasonId) {
  const result = await seasonRepository.findWorksBySeasonId(seasonId);
  return result;
};
...
export async function getSeasonWorkById(seasonId, workId) {
  ...
  const formattedWorks = await seasonRepository.findWorksBySeasonId(seasonId);
